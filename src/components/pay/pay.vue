		...
		<div id="coupon-wrapper" :class="{active:showCouponFlag}" @click.self="closeCoupon">
			<div class="coupons">
				...
				<div class="weui-cells weui-cells_checkbox coupons-content clear">
				  	<label v-for="(item,index,key) in couponsList" @click.self="changeCoupon(item.couponAmt,item.couponNo)" class="weui-cell weui-check__label" :for="joinId(item.id)">
...
<script>
	...
	export default{
		...
		methods:{
			...
			changeCoupon:function(_price,_id){//选择优惠券
				        ...
				        if(_body.code != 200){
	                    	...
	                    	if(_this.payMode == 0){//微信支付
		                    	...
		                    	if(_this.appidParams && _this.payAble){
		                    		_this.weixinPay(_data.outOrderNo);
		                    	}else{
		                    		 $.alert('微信支付发起参数错误');
		                    	}
	                    	}
			...
			weixinPay:function(orderNo){
				var _this = this;
	            function onBridgeReady(){
	                WeixinJSBridge.invoke('getBrandWCPayRequest', _this.appidParms,function(res){
	                    //initBox();
